  ...
  describe "#eval" do
    context "when given an issue field comparison" do
      ...
      it "returns issues that match the given value" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue]).eval("key = 'ISSUE-101'")
        expect(issues).to eq([issue])
      end

      it "filters out issues that do not match the given value" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue]).eval("key = 'ISSUE-102'")
    ...
    context "when given a custom field comparison" do
      ...
      it "returns issues that match the given value" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue]).eval("MyField = 'foo'")
        expect(issues).to eq([issue])
      end

      it "filters out issues that do not match the given value" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue]).eval("MyField = 'bar'")
        expect(issues).to be_empty
      end
    end
    ...
    context "when given a disjunction" do
      ...
      it "returns issues that match the disjunction" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue_a, issue_b, issue_c]).eval("MyField = 'A' or MyField = 'B'")
    ...
    context "when given a conjunction" do
      ...
      it "returns issues that match the conjunction" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue_a, issue_b, issue_c]).eval("FieldA = 'foo' and FieldB = 'bar'")
    ...
    context "when given a nested expression" do
      ...
      it "returns the issues that match the expression" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue_a, issue_b, issue_c]).eval("(FieldA = 'foo' and FieldB = 'bar') or (FieldB = 'baz' and FieldA = 'bar')")
    ...
    context "when given a negated expression" do
      ...
      it "negates the expression" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue_a, issue_b, issue_c]).eval("not MyField = 'A'")
        expect(issues).to eq([issue_b, issue_c])
      end

      it "negates compound expressions" do
        issues = JiraTeamMetrics::MqlInterpreter.new([issue_a, issue_b, issue_c]).eval("not (MyField = 'A' or MyField = 'B')")
